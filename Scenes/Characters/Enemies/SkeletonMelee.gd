...
func _ready():
	...
	detection_box.connect("area_entered", self, "_on_DetectionBox_area_entered")
	undetection_box.connect("area_exited", self, "_on_UnDetectionBox_area_exited")
...
func _select_random_weapon(): #pick a random weapon from the weapon container
	current_weapon = weapon_container.get_child(rand_range(0, weapon_container.get_child_count()))
	for child in weapon_container.get_children():
		if child != current_weapon:
			...
			child.visible = true
	attack_range.start()
	attack_range.connect("area_exited", self, "_on_player_outta_attack_range")
...
func _show_damage_indicator(damage):
	var damage_indicator_instance = damage_indicator.instance()
	damage_indicator_instance.rect_global_position = self.global_position + Vector2(rand_range(-20, 0), rand_range(-10, -30))
...
func temp_change_state(new_state, time):
	if !ai_container.get_node("TempTimer").is_connected("timeout", self, "_on_temp_state_timer"):
		ai_container.get_node("TempTimer").connect("timeout", self, "_on_temp_state_timer")
