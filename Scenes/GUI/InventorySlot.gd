...
onready var split_popup = preload("res://Scenes/GUI/SplitStackPopUp.tscn")
...
#Quick transfer with ctrl+click
	if  ContainerLoot.current_container_ID != null and CommonFunctions.UI_node.get_node_or_null("Container"):
		if event is InputEventMouseButton:
			if event.button_index == BUTTON_LEFT and event.pressed and Input.is_action_pressed("quick stack"):
...
func drop_data(_pos, data):
	if is_instance_valid(data["origin_node"]):
		...
		elif Input.is_action_pressed("split") and data["origin_stackable"] == true:
